...
TELEGRAM_ENABLED = False # Force Disabled to prevent conflicts with Dashboard/EvoLoop
...
# PHASE 1: TOP 8 LIQUIDITY (Concentrated Signal)
ALLOWED_ASSETS = [
...
REGIME_PERMISSIONS = {
    'NANO': {  # FOR $0-$49 ACCOUNTS (AGGRESSIVE MODE)
        ...
        'max_exposure_ratio': 5.0,        # UNLEASHED: 5x Total Exposure
        'max_leverage': 3.0,              # ADAPTIVE CAP: 3x
    ...
    'MICRO': {  # UNLOCKED FOR ACTIVE TRADING
        ...
        'max_exposure_ratio': 5.0,        # UNLEASHED: 5x Total Exposure
        'max_leverage': 5.0,              # ADAPTIVE CAP: 5x
    ...
    'SMALL': {
        'max_positions': 5,               # REDUCED: 5 Max (User Request)
        'max_stacks': 3,                  # Reduced stacking
        'max_exposure_ratio': 3.0,        # Conservative: 3x Total Exposure
        'max_leverage': 3.0,              # DE-LEVERAGED: 3x (Safety First)
        'allowed_pairs': ALLOWED_ASSETS,
        'correlation_check': True,
        'min_order_value': 10.0,
        'max_order_value_pct': 0.10,      # REDUCED: 10% Allocation Cap
        'cooldown_after_failure': 300,    # EXTENDED: 5m Cooldown
    },
    'MEDIUM': {
        'max_positions': 24,
        'max_stacks': 2,
        'max_exposure_ratio': 10.0,       # UNLEASHED: 10x Total Exposure
        'max_leverage': 20.0,             # ADAPTIVE CAP: 20x
        'allowed_pairs': ALLOWED_ASSETS,
        'correlation_check': True,
...
# GOVERNOR_MIN_STACK_DIST moved to GOVERNANCE section to avoid duplicates

# Reduced Concentration for Micro Accounts
MICRO_MAX_POSITIONS = 5 # User Request (Was 3)
...
MICRO_HARD_LEVERAGE_LIMIT = 3.0             # Cap at 3x (User Request)
PREDATOR_TRAILING_STOP_ATR_MULT = 3.5     # 3.5x ATR Trailing Stop (Widened from 2.0 for volatility)


# === POSITION SIZING & RISK (UNLEASHED) ===
MIN_ORDER_VALUE = 5.0                     # Raised to $5.0 to eliminate dust positions
KELLY_HARD_CAP_MARGIN = 10.0              # PHASE 1: Max $10 Initial Margin per trade (Safety)
SIZE_MAX_ALLOCATION = 0.10                # Max 10% of Equity per Trade (User Request)
MAX_RISK_PCT = 0.02                       # Conservative: 2% Risk per trade (Was 5%)
...
# Position Management Limits
POSITION_LIMITS = {
    'max_positions': 25, # UNLEASHED: Raised from 8 to allow full Regime Capacity
    'max_correlation': 0.3,
    'size_limits': {
        # Using simple symbol checks or generic logic for now
        'DEFAULT': 0.25,     # UNLEASHED: Max 25% NV per asset
        'LARGE_CAP': 0.35,   # BTC/ETH/SOL can be 35%
        'MEME': 0.15         # PEPE/DOGE max 15%
    },
...
MICRO_GUARD_GROSS_LEVERAGE = 3.0          # CAP: 3x (User Request)
...
# === GOVERNANCE / RISK ===
GOVERNOR_COOLDOWN_SECONDS = 300    # EXTENDED: 5m Cooldown (User Request)
GOVERNOR_MIN_STACK_DIST = 0.0050  # 0.50% (Raised from 0.25% - Anti-Clustering)
STACK_TIMEOUT_SECONDS = 900       # 15 minutes (Relaxed from 5m) - Max time to hold before force-reduce check
GOVERNOR_MAX_MARGIN_PCT = 0.85  # UNLEASHED: 85% Margin (Was 80%)
GOVERNOR_STACK_DECAY = 0.8
GOVERNOR_MAX_TREND_AGE_HOURS = 24.0
GOVERNOR_TREND_DECAY_START = 12.0
MAX_SIMULTANEOUS_POSITIONS = 5    # REDUCED: 5 Max (User Request)

# --- NEW: SYSTEM RECALIBRATION (REC-2026-01-31-PARTICIPATE) ---
GLOBAL_BIAS_THRESHOLD = 0.10      # WAS: 0.20 (Relax 50%)
FAIR_WEATHER_MIN_BIAS = 0.15      # WAS: 0.20 (Unlock Whale Signals)
FUNDING_POISON_LIMIT = 750.0      # WAS: 500.0 (Capture Edge Arb)
THESIS_FAILURE_ACTION = "FLAT"    # ACTION: "FLAT" = Close Position, "WARN" = Log Only
THESIS_FAILURE_TOLERANCE = 2      # Grace period in cycles
THESIS_FAILURE_TOLERANCE = 2      # Grace period in cycles
MIN_CONVICTION_FOR_ENTRY = 0.80   # PHASE 1: High Conviction Only (Was 0.25)
WHALE_BONUS_CONVICTION = 0.25     # Strengthen whale weight
MAX_POSITION_SIZE_PCT = 0.20      # CAP: 20% (Prev: 15% / Unbounded Kelly)
MAX_ENTRIES_PER_CYCLE = 2         # CAP: 2 New Entries Max
MIN_SECONDS_BETWEEN_STACKS = 60   # TIMEOUT: No same-cycle stacking
STACK_SNOOZE_DURATION = 120       # 2min vs 5min
...
WHALE_ORDER_IMBALANCE_RATIO = 5.0     # 5x Buying Pressure (Strong Bid Wall)
...
ACC_SANITY_THRESHOLD = 0.20               # >20% instant drop = Likely Data Glitch protection)

# === ARBITRAGE PARAMETERS ===
ENABLE_ARBITRAGE = False               # USER REQUEST: DISABLED (or Throttled)
ARB_SPATIAL_THRESHOLD = 0.008          # 0.8% spread
ARB_FUNDING_THRESHOLD = 0.0005         # 0.05% per 8h (High filter ~50% APY)
ARB_MAX_EXPOSURE = 0.10                # Reduced to 10%
ARB_COOLDOWN_SECONDS = 300             # 5m Cooldown
...
PERSONALITY_DOGE_RVOL = 1.2  # Stabilized: Raised to 1.2 to filter noise
